...
const socialLinks = [
  ...
  {
    href: 'www.linkedin.com/in/cagitadian',
    icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <section className="section" id="contact">
        <div className="container lg:grid lg:grid-cols-2 lg:items-stretch ">
            <div className="mb-12 lg:mb-0 lg:flex lg:flex-col">
                <h2 className="headline-2 lg:max-w-[20ch] reveal-up">Contact me for collaboration</h2>

                <p className="mt-3 mb-8 text-zinc-400 max-w-[50ch] lg:max-w-[40ch] reveal-up">
                    Reach out today to discuss your project needs and start collaborating on something amazing!
                </p>

                <div className="flex items-center gap-2 mt-auto">
                    {socialLinks.map(({href, icon}, key) =>(
                        <a key={key} href={href} target="_blank" className="grid w-12 h-12 rounded-lg place-content-center ring-inset ring-2 ring-zinc-50/5 transition-[background-color, color] hover:bg-zinc-50 hover:text-zinc-950 active:bg-zinc-50/80 reveal-up">{icon}</a>
                    ))}
                </div>

            </div>

            <form action="https://getform.io/f/boloqvxa" method="POST" className="xl:pl-10 2xl:pl-20">

                <div className="md:grid md:items-center md:grid-cols-2 md:gap-2">
                    <div className="mb-4">
                        <label htmlFor="name" className="label reveal-up">Name</label>
                        <input type="text" name="name" autoComplete="name" id="name" className="text-field reveal-up" placeholder="Cagita Dian" required />
                    </div>

                    <div className="mb-4">
                        <label htmlFor="email" className="label reveal-up">Email</label>
                        <input type="email" name="email" autoComplete="email" id="email" className="text-field reveal-up" placeholder="cagitadianayunin@gmail.com" required />
                    </div>
                </div>
                ...
                <div className="mb-4">
                    ...
                    <textarea name="message" id="message" placeholder="Hi Cagita!" required className="resize-y text-field reveal-up min-h-32 max-h-80"></textarea>
                </div>
